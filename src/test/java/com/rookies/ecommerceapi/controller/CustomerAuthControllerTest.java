...
public class CustomerAuthControllerTest {
    ...
    public void authenticateCustomerTest() throws Exception {
        ...
        this.mockMvc.perform(post("/customer/auth/signin").contentType(APPLICATION_JSON_UTF8).content(signInRequest))
    ...
    //     when(userRepository.save(Mockito.any())).thenReturn(user.get());
    //     when(customerRepository.save(Mockito.any())).thenReturn(true);
        

    //     String signUpRequest = "{\"username\": \"custom10\", \"password\": \"Qwert1!@\", \"name\": \"Test\"}";
    //     this.mockMvc.perform(post("/customer/auth/signup").contentType(APPLICATION_JSON_UTF8).content(signUpRequest))
    //             .andExpect(status().is(200));
    // }
}
